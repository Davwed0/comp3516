...
def get_br(csi_data, fs=100):
    ...
    s_t_complex = [np.mean(csi_row[20:50]) for csi_row in csi_data]
    ...
    try:
        ...
        br = fs / x[0] * 60
        if br < 8 or br > 25:   # For unrealistic values, try next peak or default to reasonable guess
            if len(x) > 1:
                br = fs / x[1] * 60
            else:
                br = 15
...
def make_csi_complex(csi_data):
    ...
    for csi_row in csi_data:
        csi_processed.append([complex(csi_row[i+1], csi_row[i]) for i in range(0, len(csi_row), 2)])
